...
export default async function Blog(params: IParams) {
  // Declare slug and blog using let or const
  const slug: string = params.params.slug;
  //console.log("in Blog Blog params" + JSON.stringify(params));
  console.log("in Blog Blog params again" + params.params.slug);
  ...
  function html() {
    return (
      <div >
        {/* The blog details */}
        <div style={{ textAlign: "center" ,width:"60%", paddingLeft:"250px"}}>
          <div style={{ textAlign: 'center' }}>
            <h1>{blog.title}</h1>
            <p>{blog.description}</p>
            <p>Published Date: <DateFormatter dateString={blog.date}/></p>
            <img src={blog.image} alt="Blog Image" style={{ width: '40%' }} />
          </div>
        </div>

        {/* Add a new comment  */}
        <div style={{ float: 'right', width:"40%", paddingRight:"50px", paddingTop: "100px"}}> 
            <div>
              <CommentForm />
            </div>
        </div>
        ...
        <h3 style = {{paddingLeft:"220px"}}>Comments:</h3>
            ...
            <div style={{ }}>
                <div style={{padding:"5px;"}}>
                  <div style={{float:"left",width:"60%",textAlign:"left", paddingLeft:"220px"}}>
                    <strong>{comment.user}</strong>&nbsp;&nbsp;&nbsp;
                    <span style={{float:"right",width:"40%",color:"GrayText",fontSize:"small"}}><DateFormatter dateString={comment.time} /></span>
                    <p>{comment.comment}</p>
                  </div>
